...
public class GaugeCircle: Gauge {
    ...
    override func getGauge(rotateAngle: Double = 0) -> CALayer {
        ...
        if bgLayer == nil {
            bgLayer = CAShapeLayer.getOval(lineWidth, path: nil, strokeStart: 0, strokeEnd: 1, strokeColor: _bgStartColor, fillColor: UIColor.clearColor(), shadowRadius: 0, shadowOpacity: 0, shadowOffsset: CGSizeZero, bounds: bounds)
            bgLayer.frame = layer.bounds
            gaugeLayer.addSublayer(bgLayer)
        }

        if ringLayer == nil {
            ringLayer = CAShapeLayer.getOval(lineWidth, path: nil, strokeStart: 0, strokeEnd: 1, strokeColor: UIColor.clearColor(), fillColor: UIColor.clearColor(), shadowRadius: ringShadowRadius, shadowOpacity: ringShadowOpacity, shadowOffsset: CGSizeZero, bounds: bounds)
